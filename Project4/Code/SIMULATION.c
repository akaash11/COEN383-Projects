...
//Page pointer
    ...
    if (Argumnt1 != 2){
        printf("*******Page Replacement Algo are: FCFS, LRU, LFU, MFU or Random can be passed as argument.\n");
        return -1;
...
printf("####################################################################\n");
	...
	int i;
        for(i=0;i<Total_PROCESS;i++) {
            ...
            Q[i].TIMEARR = rand()%60;
            Q[i].DRUTION = rand() % PROCss_DuraTN;// duration of maximum process
        ...
        for(TimeStamp = 0; TimeStamp < TotaL_DURATION; TimeStamp++) {
            ...
            //looks for a new process at start of every second
            while(index < Total_PROCESS && Q[index].TIMEARR <= TimeStamp) {

		 // checks atleast four pages
                if(PageWhoRFree(&pl,4)) {
			// if it is present bring it to the memory
