...
int main()
{
    ...
    signal(SIGALRM, alarmHandler); // Set the alarm signal handler
    ...
    while (!is_timeout)
    {
        ...
        if (process_id > 0)
        {
            ...
            selected_pipe = select(FD_SETSIZE, &set_fds, NULL, NULL, NULL);
            if (selected_pipe < 0)
            {
                perror("Select error");
                exit(1);
            }
            else if (selected_pipe == 0)
            {
            ...
            else
            {
                for (i = 0; i < NUM_PIPES; i++)
                {
                    ...
                    {
                        readFromPipe(pipes[i], pipes[i][READ_END], i);
                    }
                }
            }
        }
